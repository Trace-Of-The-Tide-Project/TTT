...
import { formatDuration } from "@/components/content/hub/atrium-types";
import { ArchiveFacets } from "./ArchiveFacets";

const PAGE_SIZE = 24;
const ROUTE_PATH = "/archive";
...
/**
 * FR-CMN-03 faceted browse over GET /articles/archive (commons-eligible
 * content only). Card/grid/pagination markup mirrors AllContentListing so the
 * archive reads as the same system as the other listing pages; the only new
 * piece is ArchiveFacets pushing filters into the URL.
 */
export async function ArchiveListing({ searchParams }: ArchiveListingProps) {
  ...
  const raw = await serverGet<Envelope>("/articles/archive", {
    viewer_lang: locale,
    page,
    limit: PAGE_SIZE,
    ...(search ? { search } : {}),
  ...
  const cardHref = (row: RawArticle) =>
    previewHrefForContentType(row.content_type ?? "article", row.id, row.slug);
      ...
      <div className="relative mx-auto w-full max-w-[1392px] px-4 py-16 sm:px-6 md:px-8">
        ...
        {totalPages > 1 && (
          <nav
            ...
            aria-label={t("archive.heading")}
          >
            <Link
              href={{ pathname: ROUTE_PATH, query: pageQuery(Math.max(1, page - 1)) }}
            ...
            <span className="text-xs tabular-nums" style={{ color: "var(--tott-salt)" }}>
              {t("archive.pageOf", { page, totalPages })}
            </span>
            <Link
              href={{ pathname: ROUTE_PATH, query: pageQuery(Math.min(totalPages, page + 1)) }}
