...
/** Ink on a gold/accent-filled control. */
...
const _HexMark = ({ size = 22 }: { size?: number }) => (
  <div
    style={{
      width: size,
      height: size * 1.15,
...
export function HomeD02({
  ...
  return (
    <div dir={dir} style={{ background: BG, color: TEXT, fontFamily: "'IBM Plex Sans', sans-serif", minHeight: "100vh" }}>
      ...
      {/* MASTHEAD DIVIDER */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 32, padding: "14px 64px", borderBottom: `1px solid ${HAIRLINE}` }}>
        ...
        <span style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: "0.22em" }}>
          2026 — MENA
        </span>
        <div style={{ width: 1, height: 14, background: HAIRLINE }} />
        <span style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: "0.22em" }}>
      ...
      <div style={{ textAlign: "center", padding: "72px 64px 56px", maxWidth: 900, margin: "0 auto" }}>
        <h1 style={{ fontSize: 76, fontWeight: 300, letterSpacing: "-0.035em", lineHeight: 1.05, color: TEXT, margin: "0 0 24px" }}>
      ...
      {spotlight ? (
        <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 64px 72px", display: "grid", gridTemplateColumns: "660px 1fr", gap: 56, alignItems: "start" }}>
          <Link href={spotlight.href} style={{ textDecoration: "none", display: "block", position: "relative" }}>
            <div style={{ height: 420, borderRadius: 4, overflow: "hidden", background: "var(--tott-home-hero-grad-mid)", position: "relative" }}>
              {spotlight.image ? (
              ...
              <span style={{ position: "absolute", top: 16, left: 16, background: GOLD, color: CTA_INK, fontSize: 10, fontWeight: 700, padding: "4px 10px", borderRadius: 2, textTransform: "uppercase", letterSpacing: "0.14em" }}>
          ...
          <div style={{ paddingTop: 8 }}>
            ...
            <Link href={spotlight.href} style={{ textDecoration: "none" }}>
              <h2 style={{ fontSize: 28, fontWeight: 400, lineHeight: 1.25, color: TEXT, margin: "12px 0 16px" }}>{spotlight.title}</h2>
            </Link>
            {spotlight.excerpt ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28 }}>
              {spotlight.authorName ? (
                <span style={{ fontSize: 13, color: MUTED }}>{spotlight.authorName}</span>
              ) : null}
            ...
            {latestThree.map((item, i) => {
              ...
              return (
                <Link key={item.id} href={href} style={{ textDecoration: "none" }}>
                  ...
                  <div style={{ fontSize: 17, fontWeight: 400, color: TEXT, lineHeight: 1.4 }}>{title}</div>
                </Link>
              );
            })}
          </div>
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "0 64px 72px", borderTop: `1px solid ${HAIRLINE}` }}>
        ...
        {STREAM_ITEMS.map((item, i) => (
          <Link
            key={item.href}
            href={item.href}
            style={{ display: "flex", alignItems: "center", gap: 32, padding: "28px 20px", borderTop: `1px solid ${HAIRLINE}`, textDecoration: "none" }}
          >
            <span style={{ fontSize: 13, color: GOLD, width: 28, flexShrink: 0 }}>0{i + 1}</span>
            <span style={{ fontSize: 22, fontWeight: 300, color: TEXT, width: 280, flexShrink: 0 }}>{ar ? item.labelAr : item.label}</span>
      ...
      {/* NEWSLETTER */}
      <section style={{ background: BG, borderTop: `1px solid ${HAIRLINE}`, padding: "60px 64px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            ...
            <h3 style={{ fontSize: 28, fontWeight: 300, color: TEXT, margin: "12px 0 16px", letterSpacing: "-0.02em" }}>
          ...
          <form onSubmit={(e) => e.preventDefault()} style={{ display: "flex", gap: 12 }}>
            <input
              ...
              dir={dir}
              style={{
                flex: 1, background: "transparent", border: "1px solid var(--tott-card-border)", borderRadius: 8,
                padding: "14px 18px", fontSize: 14, color: TEXT, outline: "none",
              }}
            />
            <button
              type="submit"
              style={{ background: GOLD, color: CTA_INK, border: "none", borderRadius: 8, padding: "14px 24px", fontSize: 14, fontWeight: 600, cursor: "pointer", flexShrink: 0 }}
            >
              {t("Subscribe", "اشترك")}
            </button>
          </form>
