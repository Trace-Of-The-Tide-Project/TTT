 ...
 * pitch, row pitch 234px = 294 card height − 60px overlap, card 276×294).
 * Rows alternate 4-up (flush start) and 5-up (bleeding past both edges by a
 * half card) — reproduced here with CSS instead of Figma's literal edge
 * "filler" hexes, by giving odd rows a negative start margin and letting
 * `overflow-hidden` on the section clip the bleed.
 *
 ...
 */
export function Honeycomb({ rows }: { rows: ReactNode[][] }) {
  return (
    <div className="hidden flex-col items-center gap-0 overflow-hidden lg:flex">
      {rows.map((row, i) => (
        <div
          key={i}
...
export function HoneycombFallback({ items }: { items: ReactNode[] }) {
  return (
