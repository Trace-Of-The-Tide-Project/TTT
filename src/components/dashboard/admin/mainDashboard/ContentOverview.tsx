...
export function ContentOverview({ rows, totalValue, manageHref }: ContentOverviewProps) {
  ...
  return (
    <div className="rounded-2xl border border-[var(--tott-card-border)] bg-[var(--tott-dash-surface)] p-5 shadow-[0_1px_2px_rgba(22,36,58,0.04),0_4px_16px_rgba(22,36,58,0.04)]">
      ...
      {/* Beach-inspired wave illustration anchoring the bottom of the table.
          Rendered via CSS mask so the wave picks up theme color via the
          parent's background-color. */}
      <span
        aria-hidden="true"
        className="pointer-events-none mx-auto mt-6 block h-16 w-full max-w-2xl opacity-60"
        style={{
          backgroundColor: "var(--tott-sea-deep)",
          WebkitMaskImage: "url(/admin/wave.svg)",
          maskImage: "url(/admin/wave.svg)",
