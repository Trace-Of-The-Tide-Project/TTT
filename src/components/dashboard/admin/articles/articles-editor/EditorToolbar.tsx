...
import { useCurrentEditor } from "./lib/editor-registry";
...
/** Default font menu mirrored from Google Docs. The visual font preview in the
 *  popover requires the font to be loaded — system fonts (Arial, Times New Roman,
 *  Verdana, etc.) render out of the box; Google Web Fonts (Roboto, Lora, …)
 *  fall back to a default until the matching `<link>` is added globally. The
 *  font-family CSS is still applied to the saved HTML regardless. */
const FONT_FAMILIES = [
  "Amatic SC",
  "Arial",
  "Calibri",
    ...
    >
      ...
      >
        {(close) => {
          ...
          const currentStyle = (editor?.getAttributes(
            inBullet ? "bulletList" : "orderedList",
          ...
          return (
            <>
              ...
              <PopoverItem active={inBullet && currentStyle === "disc"} onClick={pickBullet("disc")}>
                ●  Disc
              </PopoverItem>
              <PopoverItem
