...
async function fetchMagazineArticles(
  locale: string,
): Promise<LatestPublishedItem[]> {
  const raw = await serverGet<Envelope<ArticleListItem>>("/articles", {
    limit: 12,
    page: 1,
    status: "published",
  ...
  return unwrapList(raw).map((a) => ({
    ...
    href: `/magazine/articles/${encodeURIComponent(a.slug)}`,
  }));
}
          ...
          tabs={{
            ...
            articles:
              magazineArticles.length > 0 ? (
                <MagazineLatestPublished items={magazineArticles} allHref={null} />
              ) : undefined,
