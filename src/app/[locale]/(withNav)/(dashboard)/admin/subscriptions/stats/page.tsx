...
export default function AdminSubscriptionStatsPage() {
  ...
  if (!stats) {
    return (
      <div className="p-6">
        <p className="text-sm" style={{ color: '#555' }}>{t('loading')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <p className="text-xs uppercase tracking-[3px] mb-0.5" style={{ color: '#cba158' }}>{t('admin')}</p>
          <h1 className="text-2xl font-bold" style={{ color: '#ededed' }}>{t('revenueStats')}</h1>
        </div>
      </div>
